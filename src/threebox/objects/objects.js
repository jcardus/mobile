...
Objects.prototype = {
  ...
  line: function(obj) {
    ...
    const straightProject = utils.lnglatsToWorld(obj.geometry)
    ...
    // material
    const material = new THREE.LineBasicMaterial({ color: 0xff0000, linewidth: 21 })
    ...
    if (!_static) {
      ...
      obj.setCoords = function(lnglat) {
        ...
        if (obj.userData.units === 'meters') {
          const s = utils.projectedUnitsPerMeter(lnglat[1])
          obj.scale.set(s, s, s)
        }
  ...
  _makeGroup: function(obj, options) {
    ...
    const isArrayOfObjects = obj.length

    if (isArrayOfObjects) {
      for (const o of obj) geoGroup.add(o)
    } else {
      geoGroup.add(obj)
    }
